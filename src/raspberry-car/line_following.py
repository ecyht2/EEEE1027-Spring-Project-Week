...
def loop():
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr",
        ...
        ret, thresh = cv2.threshold(blur, threshold[0], threshold[1],
        ...
        """Find x-axis centroid of largest contour and cut power to appropriate
        motor to recenter camera on centroid.
        ...
        for name, colour in colours.items():
            if len(colour["contours"]) > 0:
                ...
                _ = int(M_c['m01']/(M_c['m00'] + 1e-5))
                area = cv2.contourArea(c_colour)
                if area > 600:
                    print("x: {}".format(x), "A: {}".format(area),
                          "c: {}".format(name))
                    cx = x
